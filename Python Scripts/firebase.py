...
def setPinState(room,device):
    status=ReadDeviceValue(room,device)
    if (status=="ON"):
...
def CamAction():
        status=ReadDeviceValue("Camera","capture")
        if status=="ON":
            capture_image("capture")
            WriteDeviceValue("Camera","capture","DONE")
        status=ReadDeviceValue("Camera","suspect")
        if status=="ON":
...
def tv_volume_up(volume):
    printText("Volume UP",volume,2.0)
    channelNum=ReadDeviceValue("TV","channel")
    printText("Channel {}".format(channelNum),channels[channelNum],0.01)
    
def tv_volume_down(volume):
    printText("Volume DOWN",volume,2.0)
    channelNum=ReadDeviceValue("TV","channel")
    printText("Channel {}".format(channelNum),channels[channelNum],0.01)
...
try:
    lcd_init()
    tvState,channelNum,volume=tv_init()
    desTemp=ReadSettingsValue("Temperature")
    
    while True:
        #devices
        for room in rooms:
            for device in rooms[room]:
                setPinState(room,device)
        #TV
        if(tvState!=ReadDeviceValue("TV","state")):
            tvState=ReadDeviceValue("TV","state")
            if(tvState=="ON"):
                tv_on()
            else:
        ...
        if(channelNum!=ReadDeviceValue("TV","channel")):
            channelNum=ReadDeviceValue("TV","channel")
            if(tvState=="ON"):
                tv_channel_update(channelNum)
        if(int(volume)>int(ReadDeviceValue("TV","volume"))):
            volume=ReadDeviceValue("TV","volume")
            if(tvState=="ON"):
                tv_volume_down(volume)
        elif(int(volume)<int(ReadDeviceValue("TV","volume"))):
            volume=ReadDeviceValue("TV","volume")
            if(tvState=="ON"):
                tv_volume_up(volume)
        ...
        #temperature
        if(ReadSettingsValue("Temperature")!=desTemp):
            desTemp=ReadSettingsValue("Temperature")
            updateTemp(desTemp)
            
        #camera
        CamAction()
        if(ReadDeviceValue("Door","knock")=="ON"):
